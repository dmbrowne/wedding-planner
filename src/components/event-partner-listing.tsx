...
export const EventPartnerListing: React.FC<{ eventId: string }> = ({ eventId }) => {
  ...
  const allEventGuestsFetched = eventGuestIdsForEvent.length === numberOfGuests - numberOfPlusOnes;
  // const desiredGuestsHaveBeenFetched = eventGuestIdsForEvent.every(eventGuestId => {
  ...
  if (!desiredGuestsHaveBeenFetched) return <Pulsar />;
  ...
  return (
    <>
      {showAllPartners && getListingButton()}
      {partners.slice(0, !showAllPartners ? 5 : undefined).map(eventGuestTuple => {
        ...
        const groupMsg = (name: string) =>
          `is part of a group, the invitation sent to ${name} will be addressed to everyone in that group`;
        return (
          <Box
            elevation="xsmall"
